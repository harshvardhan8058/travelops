...
// ---------------------------------------------------------------- passenger view
/*
 * `GET /passenger/{booking_ref}/disruption`.
 *
 * The same recorded state the console reads, addressed by booking reference. Three absences are
 * deliberate and are the reason these types live here rather than being widened later:
 *
 *   - there is no name, email or phone field, so none can be rendered;
 *   - there is no money field, because entitlements are the policy engine's answer;
 *   - an alternative flight's `basis` cannot say a seat exists, only that the schedule is reachable.
 */

export type PassengerSegmentStatus = 'on_time' | 'scheduled' | 'delayed' | 'cancelled' | 'at_risk';

export type PassengerActionState =
  'succeeded' | 'failed' | 'needs_human' | 'executing' | 'awaiting_approval' | 'pending';

/** Whether a piece of work names this booking or covers the whole incident. */
export type PassengerActionScope = 'this_booking' | 'incident';

export type PassengerNextStepState =
  'awaiting_approval' | 'executing' | 'resolved' | 'monitoring' | 'no_disruption';

export interface PassengerSegmentOut {
  segment_id: number;
  ...
  /** Null when the airline has published no revision. Never the scheduled time repeated. */
  estimated_departure: string | null;
  /** Null when nothing is published; `0` means published and on time. Different claims. */
...
export interface PassengerDisruption {
  ...
  flight_number: string;
  airport_icao: string;
  /** The operational trigger as recorded. Never a verdict about who is at fault. */
...
export interface PassengerConnectionImpact {
  ...
  connection_airport_icao: string;
  inbound_scheduled_arrival: string;
  inbound_revised_arrival: string;
  onward_scheduled_departure: string;
  minimum_connection_minutes: number;
  /** Negative: how far short of the minimum the turnaround falls. */
  shortfall_minutes: number;
  recovered_by_onward_delay: boolean;
  /** The action that established this, so the claim is traceable. */
  established_by_action_id: number;
}
...
export type PassengerOptionKind = 'alternative_flight' | 'hotel_room';

/**
 * `basis` is the whole guarantee. No value means "a seat is available", because this system holds
 * no capacity data; `schedule_feasible_only` means the departure is late enough to be reachable.
 */
export type PassengerOptionBasis =
  'schedule_feasible_only' | 'recorded_reservation' | 'simulated_reservation';

export interface PassengerOption {
  kind: PassengerOptionKind;
  label: string;
  basis: PassengerOptionBasis;
...
export interface PassengerAction {
  ...
  applies_to: PassengerActionScope;
  /** Null while nothing has executed. Never a fabricated timestamp. */
  at: string | null;
  reason_code: string | null;
  approval_scope: 'action' | 'plan' | null;
  awaiting_human: boolean;
}

export interface PassengerNextStep {
  state: PassengerNextStepState;
  driven_by_action_type: string | null;
  /** Always null: no deadline is recorded anywhere, so none is shown. */
  respond_by: null;
}

export interface PassengerDisruptionResponse {
  booking_ref: string;
  /** Pseudonymous. `PAX-00001`, never a name. */
  passenger_reference: string;
  cabin: string;
  tier: string;
  has_special_needs: boolean;
  trip: PassengerTrip;
  /** Null when no incident touches any segment. A recorded, undisrupted trip is an answer. */
  disruption: PassengerDisruption | null;
  connection: PassengerConnectionImpact | null;
  priority: PassengerPriority | null;
