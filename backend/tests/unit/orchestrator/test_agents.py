...
class TestTheFixturePath:
    """`LLM_MODE=fixture` must exercise parse, validate and reflect without a network call."""

    async def test_the_committed_response_produces_a_usable_plan(self, monkeypatch):
        from app.config import LLMMode

        monkeypatch.setattr("app.llm.client.get_modes", lambda: _modes(LLMMode.fixture))
        proposal = await planner.propose(
            planner.PlanningFacts(
                ...
                severity="high",
                target_refs=REFS,
                available_actions=REGISTERED | {ActionType.find_hotel_options},
            )
        )
        assert proposal is not None
        actions = [task.action for task in proposal.tasks]
        assert ActionType.check_connections in actions
        # The fixture deliberately proposes reassign_gate, which has no service here.
        assert ActionType.reassign_gate not in actions
        assert "reassign_gate" in proposal.reflection.dropped_actions

    async def test_the_model_self_report_is_recorded_and_not_acted_on(self, monkeypatch):
        from app.config import LLMMode
        ...
        assert proposal is not None
        assert proposal.audit.model_self_report == 82
        # Recorded, and the plan still went through reflection like any other.
        assert proposal.tasks

    async def test_llm_off_never_calls_and_yields_the_playbook(self, monkeypatch):
        from app.config import LLMMode

        monkeypatch.setattr("app.llm.client.get_modes", lambda: _modes(LLMMode.off))
        proposal = await planner.propose(
            planner.PlanningFacts(
                incident_reference="INC-1",
                trigger_type="weather",
    ...
    async def test_no_registered_service_means_no_proposal(self):
        proposal = await planner.propose(
            planner.PlanningFacts(
                incident_reference="INC-1",
                trigger_type="weather",
...
class TestNeedsHumanIsARequestNotAControl:
    async def test_it_falls_back_rather_than_blocking(self, monkeypatch):
        from app.agents.contract import ModelCallAudit
        from app.llm.client import LLMResult

        async def _needs_human(**_kwargs):
            return LLMResult(
                payload={
                    "status": ActionStatus.needs_human.value,
                    "reason": "this needs a person's judgement",
                    "payload_type": "planner.v1",
                    "tasks": [{"action": "check_connections", "target_refs": []}],
                },
                audit=ModelCallAudit(generator="planner-agent"),
                source="fixture",
            )

        monkeypatch.setattr("app.llm.client.complete_json", _needs_human)
        proposal = await planner.propose(
            planner.PlanningFacts(
                incident_reference="INC-1",
                trigger_type="weather",
        ...
        # No plan from the agent, and nothing blocked: the playbook runs and the ask is logged.
        assert proposal is None


class TestMalformedOutputIsRejectedNotRepaired:
    async def test_it_gives_up_after_the_retry(self, monkeypatch):
        from app.agents.contract import ModelCallAudit
        from app.llm.client import LLMResult

        calls: list[int] = []

        async def _garbage(**_kwargs):
            calls.append(1)
            return LLMResult(
                payload={"status": "success", "tasks": "not a list"},
                audit=ModelCallAudit(generator="planner-agent"),
                source="live",
            )

        monkeypatch.setattr("app.llm.client.complete_json", _garbage)
        proposal = await planner.propose(
            planner.PlanningFacts(
                incident_reference="INC-1",
                trigger_type="weather",
        ...
        assert proposal is None
        assert len(calls) == planner.MAX_ATTEMPTS, "one retry, then the deterministic path"
...
class TestTheToolInvocationBoundary:
    ...
    def test_the_llm_client_is_the_only_module_that_talks_to_a_model(self):
        offenders = [
            path.relative_to(APP)
            for path in APP.rglob("*.py")
            if path.name != "client.py" and "api.groq.com" in path.read_text(encoding="utf-8")
        ]
